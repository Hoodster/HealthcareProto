...
class BasicRetrieverService(RetrieverService):
    ...
    def add(self, items: List[Dict]):
        ...
        vecs = self._ensure_normalized(vecs).astype(np.float32)
        # FAISS expects shape (n, dim)
        # FAISS Python bindings for IndexFlatIP: add(x: np.ndarray)
        # Ensure contiguous float32 array
        vecs = np.ascontiguousarray(vecs, dtype=np.float32)
        idx: Any = cast(Any, self._index)
        idx.add(vecs)  # type: ignore[call-arg]
